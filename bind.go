...
func (b *CustomBinder) BindBody(c echo.Context, i interface{}) (err error) {
	...
	case strings.HasPrefix(ctype, echo.MIMEMultipartForm):
		...
		if err = b.bindData(i, form.Value, "formData"); err != nil {
			return echo.NewHTTPError(http.StatusBadRequest, err.Error()).SetInternal(err)
		}
		if err = b.bindFile(i, form.File, "formData"); err != nil {
			return echo.NewHTTPError(http.StatusBadRequest, err.Error()).SetInternal(err)
		}
	default:
...
func (b *CustomBinder) bindFile(destination interface{}, data map[string][]*multipart.FileHeader, tag string) error {
	...
	for i := 0; i < typ.NumField(); i++ {
		...
		if structFieldKind == reflect.Slice {
			val.Field(i).Set(reflect.ValueOf(inputValue))
		} else if numElems > 0 {
			val.Field(i).Set(reflect.ValueOf(inputValue[0]))
		}
	}
	return nil
}
...
func (b *CustomBinder) BindPathParams(c echo.Context, i interface{}) error {
	...
	if err := b.bindData(i, params, "path"); err != nil {
		return echo.NewHTTPError(http.StatusBadRequest, err.Error()).SetInternal(err)
	}
	return nil
}
...
func (b *CustomBinder) BindCookies(c echo.Context, i interface{}) error {
	...
	if err := b.bindData(i, cookies, "cookie"); err != nil {
		return echo.NewHTTPError(http.StatusBadRequest, err.Error()).SetInternal(err)
	}
	return nil
}
